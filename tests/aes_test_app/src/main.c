...
static const char *test_name(enum aes_test_id id) {
  switch (id) {
  case TEST_ECB_128:
    return "ECB-128 encrypt/decrypt";
  case TEST_ECB_256:
    return "ECB-256 encrypt/decrypt";
  case TEST_CBC_128:
    return "CBC-128 encrypt/decrypt";
  case TEST_CBC_256:
    return "CBC-256 encrypt/decrypt";
  case TEST_CTR_128:
    return "CTR-128 encrypt/decrypt";
  case TEST_CTR_256:
    return "CTR-256 encrypt/decrypt";
  default:
    return "UNKNOWN";
  }
}
...
static const uint8_t ecb_key128[16] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae,
...
static const uint8_t ecb_plaintext[16] = {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40,
...
static const uint8_t cbc_iv[16] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
...
static const uint8_t ctr_iv[16] = {0xf0, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5,
...
static void run_ecb_enc_dec(const struct device *dev, const uint8_t *key,
                            size_t key_len, const uint8_t *expected_ct,
                            const char *label) {
  int ret;
  ...
  ret = cipher_begin_session(dev, &enc_ctx, CRYPTO_CIPHER_ALGO_AES,
  ...
  ret = cipher_begin_session(dev, &dec_ctx, CRYPTO_CIPHER_ALGO_AES,
...
static void run_cbc_enc_dec(const struct device *dev, const uint8_t *key,
                            size_t key_len, const uint8_t *expected_ct,
                            const char *label) {
  int ret;
  uint8_t iv_enc[16];
...
static void run_ctr_enc_dec(const struct device *dev, const uint8_t *key,
                            size_t key_len, const uint8_t *expected_ct,
                            const char *label) {
  int ret;
  uint8_t ctr_enc[16];
...
ZTEST(aes_ecb_tests, test_ecb_128) {
  ...
  run_ecb_enc_dec(dev, ecb_key128, sizeof(ecb_key128), ecb_expected_ct_128,
                  "AES-128 ECB");
...
ZTEST(aes_ecb_tests, test_ecb_256) {
  ...
  run_ecb_enc_dec(dev, ecb_key256, sizeof(ecb_key256), ecb_expected_ct_256,
                  "AES-256 ECB");
